...
- Sex/gender-specific question filtering (`EligibilityCriterion.AppliesToSex`, `AdaptiveQuestionService`'s `not_applicable` status) depends on Claude tagging each criterion at extraction time, backed by a deterministic keyword backstop (`SexApplicabilityHeuristics`) covering a curated term list (pregnancy, breastfeeding, prostate, etc.) — not exhaustive medical knowledge. A criterion outside that list that Claude also fails to tag simply keeps the pre-existing behavior (asked to every patient), never a crash. Gating only ever skips a criterion on a clear, confident sex mismatch — an unanswered or "Intersex or Other" response never skips anything.
